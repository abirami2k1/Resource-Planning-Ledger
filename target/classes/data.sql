...
-- 11) Posting rules (pool withdrawal → alert memo) for F8 smoke data
INSERT INTO posting_rule (id, trigger_account_id, output_account_id, strategy_type) VALUES
  (1, 1, 9, 'UNDER_BALANCE_ALERT'),
  (2, 2, 9, 'UNDER_BALANCE_ALERT'),
  (3, 3, 9, 'UNDER_BALANCE_ALERT')
ON CONFLICT (id) DO NOTHING;
...
SELECT setval('posting_rule_id_seq', (SELECT COALESCE(MAX(id), 1) FROM posting_rule), true);
